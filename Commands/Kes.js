...
module.exports.run = async(client, message, args, embed) => {
    if (ayar.roles.enAltYt.some(s => !message.member.roles.cache.has(s)) && !message.member.hasPermission(8)) return message.channel.send(embed.setDescription(`${message.author}, Bu komutu kullanmak için yeterli yetkiye sahip değilsin!`)).sil(7);
...
exports.config = {
    name: "kes",
    usage: `${ayar.bot.botPrefix}kes [@Noâ/ID]`,
    guildOnly: true,
    aliases: [],
    cooldown: 3000
};